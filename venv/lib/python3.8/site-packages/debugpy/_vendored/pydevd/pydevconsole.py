...
class Command:
    ...
    def symbol_for_fragment(self):
        return (
            'single'
            if not self.is_single_line and IS_JYTHON or self.is_single_line
            else 'exec'
        )
...
def init_set_return_control_back(interpreter):
    ...
    def return_control():
        ...
        return not interpreter.exec_queue.empty()
...
def process_exec_queue(interpreter):
    ...
    while 1:
        if kill_if_pid_not_alive != -1 and not pid_exists(
            kill_if_pid_not_alive
        ):
            exit()
...
def exec_code(code, globals, locals, debugger):
    ...
    if res := interpreterInterface.need_more(code):
        return True
