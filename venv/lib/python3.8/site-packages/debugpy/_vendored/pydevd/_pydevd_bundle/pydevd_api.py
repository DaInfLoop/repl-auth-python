        ...
        def iterate():
            ...
            for const in code.co_consts:
                if isinstance(const, types.CodeType) and const.co_filename == code.co_filename:
                    yield from _get_code_lines(const)
...
class PyDevdAPI(object):
    ...
    def set_ide_os_and_breakpoints_by(self, py_db, seq, ide_os, breakpoints_by):
        ...
        '''
        py_db._set_breakpoints_with_id = breakpoints_by == 'ID'
    ...
    def request_step(self, py_db, thread_id, step_cmd_id):
        if t := pydevd_find_thread_by_id(thread_id):
            py_db.post_method_as_internal_command(
                thread_id,
    ...
    def to_str(self, s):
        ...
        if s.__class__ != str:
            raise AssertionError(
                f'Expected to have str on Python 3. Found: {s} ({s.__class__})'
            )
        return s
    ...
    def filename_to_str(self, filename):
        ...
        if filename.__class__ != str:
            raise AssertionError(
                f'Expected to have str on Python 3. Found: {filename} ({filename.__class__})'
            )
        return filename
    ...
    def add_breakpoint(
        ...
        '''
        assert (
            original_filename.__class__ == str
        ), f'Expected str, found: {original_filename.__class__}'
    ...
    def remove_all_breakpoints(self, py_db, received_filename):
        ...
        if received_filename == '*':
            ...
            for canonical_normalized_filename, file_to_id_to_breakpoint in itertools.product(translated_filenames, lst):
                if canonical_normalized_filename in file_to_id_to_breakpoint:
    ...
    def set_function_breakpoints(self, py_db, function_breakpoints):
        function_breakpoint_name_to_breakpoint = {
            function_breakpoint.func_name: function_breakpoint
    ...
    def get_decompiled_source_from_frame_id(self, py_db, frame_id):
        ...
        return ''.join(lines) or code_to_bytecode_representation(frame.f_code)
    ...
    def remove_plugins_exception_breakpoint(self, py_db, exception_type, exception):
        ...
        if supported_type := plugin.remove_exception_breakpoint(
    ...
    def set_source_mapping(self, py_db, source_filename, mapping):
        ...
        if error_msg := py_db.source_mapping.set_source_mapping(
    ...
    def _get_windows_ppid(self):
        this_pid = os.getpid()
        return next(
            (ppid for ppid, pid in _list_ppid_and_pid() if pid == this_pid), None
        )
    ...
    def _terminate_child_processes_windows(self, dont_terminate_child_pids):
        ...
        for _ in range(50):  # Try this at most 50 times before giving up.
            ...
            if children_pids := [
                ...
                if ppid == this_pid and pid not in dont_terminate_child_pids
            ]:
    ...
    def _popen(self, cmdline, **kwargs):
        try:
            ...
            if DebugInfoHolder.DEBUG_TRACE_LEVEL >= 1:
                pydev_log.exception(f"Error running: {' '.join(cmdline)}")
            return None
