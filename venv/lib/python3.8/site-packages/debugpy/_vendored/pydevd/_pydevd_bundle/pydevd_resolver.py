...
TOO_LARGE_MSG = (
    f'Too large to show contents. Max items to show: {MAX_ITEMS_TO_HANDLE}'
)
...
def sorted_attributes_key(attr_name):
    if attr_name.startswith('__'):
        return (3, attr_name) if attr_name.endswith('__') else (2, attr_name)
...
class DefaultResolver:
    ...
    def get_contents_debug_adapter_protocol(self, obj, fmt=None):
        ...
        eval_name = '.__dict__[%s]' if used___dict__ else '.%s'
        ret = []
    ...
    def get_names(self, var):
        ...
        try:
            ...
            names = []
        if not names and hasattr_checked(var, '__dict__'):
    ...
    def _get_py_dictionary(self, var, names=None, used___dict__=False):
        ...
        for name in names:
            try:
                ...
                attr = var.__dict__[name] if used___dict__ else getattr(var, name)
                # filter functions?
                if filter_function and (
                    inspect.isroutine(attr) or isinstance(attr, MethodWrapperType)
                ):
                    continue
            except:
...
def _does_obj_repr_evaluate_to_obj(obj):
    ...
    try:
        if isinstance(obj, tuple):
            return all(_does_obj_repr_evaluate_to_obj(o) for o in obj)
        else:
...
class DictResolver:
    ...
    def get_contents_debug_adapter_protocol(self, dct, fmt=None):
        ...
        for key, val in dct.items():
            ...
            if key_as_str in found_representations:
                ...
                key_as_str = f'{key_as_str} (id: {id(key)})'
        ...
        if from_default_resolver := defaultResolver.get_contents_debug_adapter_protocol(
            dct, fmt
        ):
    ...
    def get_dictionary(self, dct):
        ...
        for key, val in dct.items():
            ...
            key = f'{self.key_to_str(key)} ({id(key)})'
            ret[key] = val
            if i > MAX_ITEMS_TO_HANDLE:
...
class TupleResolver:  # to enumerate tuples and lists
    ...
    def get_contents_debug_adapter_protocol(self, lst, fmt=None):
        ...
        format_str = f'%0{len(str(l - 1))}d'
        if fmt is not None and fmt.get('hex', False):
            format_str = '0x%0' + str(len(hex(l).lstrip('0x'))) + 'x'

        for i, item in enumerate(lst):
            ret.append((format_str % i, item, f'[{i}]'))
...
class SetResolver:
    ...
    def resolve(self, var, attribute):
        ...
        raise UnableToResolveVariableException(
            f'Unable to resolve {attribute} in {var}'
        )
...
class JyArrayResolver:
    ...
    def resolve(self, var, attribute):
        return None if attribute == GENERATED_LEN_ATTR_NAME else var[int(attribute)]

    def get_dictionary(self, obj):
        ret = {i: obj[i] for i in range(len(obj))}
...
class FrameResolver:
    ...
    def resolve(self, obj, attribute):
        ...
        return obj.f_locals if attribute == 'f_locals' else None
    ...
    def get_frame_name(self, frame):
        ...
        try:
            ...
            return f'frame: {frame.f_code.co_name} [{name}:{frame.f_lineno}]  id:{id(frame)}'
        except:
            return 'frame object'
...
def get_var_scope(attr_name, attr_value, evaluate_name, handle_return_values):
    ...
    if (
        ...
        and attr_name == RETURN_VALUES_DICT
        or attr_name == GENERATED_LEN_ATTR_NAME
    ):
        return ''
