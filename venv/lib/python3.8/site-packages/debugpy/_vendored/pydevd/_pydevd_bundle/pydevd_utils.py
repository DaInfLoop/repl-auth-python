    ...
    try:
        ...
        for t in threading_enumerate():
            ...
            thread_id_to_name_and_is_pydevd_thread[t.ident] = (
                f'{t.name}  (daemon: {t.daemon}, pydevd thread: {is_pydevd_thread})',
                is_pydevd_thread,
            )
    except:
        pass
        ...
        for i, (filename, lineno, name, line) in enumerate(traceback.extract_stack(frame)):
            ...
            if i == 0 and 'self' in frame.f_locals:
                ...
                try:
                    ...
                    stream.write(f"Unable to get str of: {type(frame.f_locals['self'])}")
                stream.write('\n')
        stream.flush()
...
def _extract_expression_list(log_message):
    ...
    for c in char_iter:
        if c == '{':
            if expression_var := _extract_variable_nested_braces(char_iter):
                expression.append('%s')
...
def convert_dap_log_message_to_expression(log_message):
    try:
        ...
        return repr(f'Unbalanced braces in: {log_message}')
    return (
        f'{repr(expression)} % ('
        + ', '.join(str(x) for x in expression_vars)
        + ',)'
        if expression_vars
...
def import_attr_from_module(import_with_attr_access):
    if '.' not in import_with_attr_access:
        ...
        raise ImportError(
            f'Unable to import module with attr access: {import_with_attr_access}'
        )
    ...
    while True:
        try:
            ...
            attr_name = f'{new_attr_part}.{attr_name}'
        else:
