...
class _Token(object):
    ...
    def mark_after(self, v):
        if isinstance(v, _Token):
            ...
            raise AssertionError(f'Unhandled: {v}')
    ...
    def __repr__(self):
        return f'Token({self.tok}, after: {self.get_after_tokens()})'
...
class _BaseHandler(object):
    ...
    def __repr__(self, *args, **kwargs):
        try:
            return f"{self.instruction} line:{self.i_line}"
        except:
...
class _MakeFunctionPy3(_BaseHandler):
    ...
    def _handle(self):
        ...
        default_node = stack.pop() if self.instruction.argval & 0x01 else None
...
def _print_after_info(line_contents, stream=None):
    ...
    for token in line_contents:
        if after_tokens := token.get_after_tokens():
            s = '%s after: %s\n' % (
                repr(token.tok),
                ('"' + '", "'.join(t.tok for t in token.get_after_tokens()) + '"'))
            stream.write(s)
        else:
            stream.write(f'{repr(token.tok)}      (NO REQUISITES)')
...
def _compose_line_contents(line_contents, previous_line_tokens):
    ...
    while line_contents:
        ...
        if not added:
            ...
            lst.extend(token.tok for token in line_contents if token not in handled)
            stream = StringIO()
