...
class ReaderThread(PyDBDaemonThread):
    ...
    def _on_run(self):
        try:
            ...
            if (
                not self._kill_received
                and sys is not None
                and pydev_log_exception is not None
            ):
                pydev_log_exception()
...
class WriterThread(PyDBDaemonThread):
    ...
    def __init__(self, sock, py_db, terminate_on_socket_close=True):
        ...
        self.timeout = 0 if pydevd_vm_type.get_vm_type() == 'python' else 0.1
...
class InternalThreadCommand(object):
    ...
    def can_be_executed_by(self, thread_id):
        ...
        '''
        return self.thread_id == thread_id or self.thread_id.endswith(f'|{thread_id}')
    ...
    def __str__(self):
        return f'InternalThreadCommands({self.method}, {self.args}, {self.kwargs})'
...
def internal_reload_code(dbg, seq, module_name, filename):
    try:
        ...
        if module_name is not None:
            ...
            if module_name not in sys.modules and '.' in module_name:
        ...
        if modules_to_reload:
            ...
            for module, module_name in modules_to_reload.values():
                _send_io_message(
                    dbg,
                    f'code reload: Start reloading module: "{module_name}'
                    + '" ... \n',
                )
...
class InternalSetNextStatementThread(InternalThreadCommand):
    ...
    def do_it(self, dbg):
        if t := pydevd_find_thread_by_id(self.thread_id):
...
def internal_get_variable_json(py_db, request):
    ...
    try:
        try:
            ...
            variables.extend(
                child_var.get_var_data(fmt=fmt)
                for child_var in variable.get_children_variables(
                    fmt=fmt, scope=scope
                )
            )
    except:
    ...
    def do_it(self, dbg):
        ...
        try:
            ...
            cmd = dbg.cmd_factory.make_error_message(
                self.sequence,
                f"Error resolving variables {get_exception_traceback_str()}",
            )
            dbg.writer.add_command(cmd)
...
class InternalGetArray(InternalThreadCommand):
    ...
    def do_it(self, dbg):
        try:
            ...
            cmd = dbg.cmd_factory.make_error_message(
                self.sequence,
                f"Error resolving array: {get_exception_traceback_str()}",
            )
            dbg.writer.add_command(cmd)
...
def internal_change_variable(dbg, seq, thread_id, frame_id, scope, attr, value):
    ...
    try:
        ...
        cmd = dbg.cmd_factory.make_error_message(
            seq,
            f"Error changing variable attr:{attr} expression:{value} traceback:{get_exception_traceback_str()}",
        )
        dbg.writer.add_command(cmd)
...
def internal_change_variable_json(py_db, request):
    ...
    if variable is None:
        _write_variable_response(
            ...
            value='',
            success=False,
            message=f'Unable to find variable container to change: {variables_reference}.',
        )
        return
    ...
    if child_var is None:
        _write_variable_response(
            ...
            value='',
            success=False,
            message=f'Unable to change: {arguments.name}.',
        )
        return
...
def internal_get_frame(dbg, seq, thread_id, frame_id):
    ...
    try:
        ...
        if frame is not None:
            ...
            cmd = dbg.cmd_factory.make_error_message(
                seq, f"Frame not found: {frame_id} from thread: {thread_id}"
            )
        dbg.writer.add_command(cmd)
    except:
        cmd = dbg.cmd_factory.make_error_message(
            seq, f"Error resolving frame: {frame_id} from thread: {thread_id}"
        )
        dbg.writer.add_command(cmd)
...
def internal_get_smart_step_into_variants(dbg, seq, thread_id, frame_id, start_line, end_line, set_additional_thread_info):
    try:
        ...
        for variant in variants:
            if variant.children_variants:
                for child_variant in variant.children_variants:
                    ...
                    xml += f'<variant name="{quote(child_variant.name)}" isVisited="{str(child_variant.is_visited).lower()}" line="{child_variant.line}" offset="{variant.offset}" childOffset="{child_variant.offset}" callOrder="{child_variant.call_order}"/>'
            else:
                xml += f'<variant name="{quote(variant.name)}" isVisited="{str(variant.is_visited).lower()}" line="{variant.line}" offset="{variant.offset}" childOffset="-1" callOrder="{variant.call_order}"/>'
        ...
        cmd = dbg.cmd_factory.make_error_message(
            seq,
            f"Error getting smart step into variants for frame: {frame_id} from thread: {thread_id}",
        )
        dbg.writer.add_command(cmd)
                ...
                if variant.children_variants:
                    for child_variant in variant.children_variants:
                        ...
                        if child_variant.call_order > 1:
                            targets.append(
                                StepInTarget(
                                    id=target_id,
                                    label=f'{child_variant.name} (call {child_variant.call_order})',
                                )
                            )
                        else:
                    ...
                    if variant.call_order > 1:
                        targets.append(
                            StepInTarget(
                                id=target_id,
                                label=f'{variant.name} (call {variant.call_order})',
                            )
                        )
                    else:
    ...
    with ctx:
        ...
        if frame_id is None:
            ...
            if is_error:
                if context == 'hover':  # In a hover it doesn't make sense to do an exec.
                    ...
                    msg = f'{eval_result.result.__class__.__name__}: {eval_result.result}'
        ...
        if frame_tracker is None:
            ...
            _evaluate_response(
                ...
                request,
                result='',
                error_message=f'Thread id: {thread_id} is not current thread id.',
            )
            return
...
def internal_evaluate_expression(dbg, seq, thread_id, frame_id, expression, is_exec, trim_if_too_big, attr_to_set_result):
    ...
    try:
        ...
        cmd = dbg.cmd_factory.make_error_message(
            seq, f"Error evaluating expression {exc}"
        )
        dbg.writer.add_command(cmd)
...
def internal_set_expression_json(py_db, request, thread_id):
    ...
    frame = py_db.find_frame(thread_id, frame_id)
    exec_code = f'{expression} = ({value})'
    result = pydevd_vars.evaluate_expression(py_db, frame, exec_code, is_exec=True)
    is_error = isinstance(result, ExceptionOnEvaluate)

    if is_error:
        _set_expression_response(
            py_db,
            request,
            result,
            error_message=f'Error executing: {exec_code}',
        )
        return
    ...
    try:
        ...
        try:
            ...
            if frame is not None:
                ...
                cmd = dbg.cmd_factory.make_error_message(
                    seq,
                    f"internal_get_completions: Frame not found: {frame_id} from thread: {thread_id}",
                )
            dbg.writer.add_command(cmd)
        finally:
...
def internal_get_description(dbg, seq, thread_id, frame_id, expression):
    ...
    try:
        ...
        description_xml = f'<xml><var name="" type="" value="{description}"/></xml>'
        cmd = dbg.cmd_factory.make_get_description_message(seq, description_xml)
        dbg.writer.add_command(cmd)
    except:
        exc = get_exception_traceback_str()
        cmd = dbg.cmd_factory.make_error_message(
            seq, f"Error in fetching description{exc}"
        )
        dbg.writer.add_command(cmd)
...
def build_exception_info_response(dbg, thread_id, request_seq, set_additional_thread_info, iter_visible_frames_info, max_frames):
    ...
    if topmost_frame is not None:
        try:
            ...
            while frames_list is not None and len(frames_list):
                ...
                for frame_id, frame, method_name, original_filename, filename_in_utf8, lineno, _applied_mapping, show_as_current_frame in iter_visible_frames_info(dbg, frames_list):

                    line_text = linecache.getline(original_filename, lineno)

                        # Never filter out plugin frames!
                    if (
                        not getattr(frame, 'IS_PLUGIN_FRAME', False)
                        and dbg.is_files_filter_enabled
                        and dbg.apply_files_filter(
    ...
    if current_paused_frame_name:
        name += f'       (note: full exception trace is shown but execution is paused at: {current_paused_frame_name})'
    ...
    return pydevd_schema.ExceptionInfoResponse(
    ...
    def do_it(self, dbg):
        try:
            ...
            for filename, line, methodname, methodobj in self.stacktrace:
                ...
                callstack += f'<frame thread_id = "{self.thread_id}" file="{makeValid(filename)}" line="{line}" name="{makeValid(methodname)}" obj="{makeValid(methodobj)}" />'
            callstack += "</xml>"
            ...
            sys.stderr.write('%s\n' % (exc,))
            cmd = dbg.cmd_factory.make_error_message(
                self.sequence, f"Error Sending Exception: {exc}"
            )
            dbg.writer.add_command(cmd)
...
class InternalSendCurrExceptionTrace(InternalThreadCommand):
    ...
    def do_it(self, dbg):
        try:
            ...
            sys.stderr.write('%s\n' % (exc,))
            cmd = dbg.cmd_factory.make_error_message(
                self.sequence, f"Error Sending Current Exception Trace: {exc}"
            )
            dbg.writer.add_command(cmd)
...
class InternalSendCurrExceptionTraceProceeded(InternalThreadCommand):
    ...
    def do_it(self, dbg):
        try:
            ...
            cmd = dbg.cmd_factory.make_error_message(
                self.sequence,
                f"Error Sending Current Exception Trace Proceeded: {exc}",
            )
            dbg.writer.add_command(cmd)
        ...
        try:
            ...
            if frame is not None:
                ...
                console_message.add_console_message(
                    ...
                    f"Select the valid frame in the debug view (thread: {self.thread_id}, frame: {self.frame_id} invalid)",
                )
                cmd = dbg.cmd_factory.make_error_message(self.sequence, console_message.to_xml())
        except:
            exc = get_exception_traceback_str()
            cmd = dbg.cmd_factory.make_error_message(
                self.sequence, f"Error evaluating expression {exc}"
            )
        dbg.writer.add_command(cmd)
...
class InternalRunCustomOperation(InternalThreadCommand):
    ...
    def do_it(self, dbg):
        try:
            ...
            exc = get_exception_traceback_str()
            cmd = dbg.cmd_factory.make_error_message(
                self.sequence, f"Error in running custom operation{exc}"
            )
            dbg.writer.add_command(cmd)
...
class InternalConsoleGetCompletions(InternalThreadCommand):
    ...
    def do_it(self, dbg):
        ...
        try:
            ...
            exc = get_exception_traceback_str()
            cmd = dbg.cmd_factory.make_error_message(
                self.sequence, f"Error in fetching completions{exc}"
            )
            dbg.writer.add_command(cmd)
...
class InternalConsoleExec(InternalThreadCommand):
    ...
    def do_it(self, dbg):
        ...
        try:
            ...
            sys.stderr.write('%s\n' % (exc,))
            cmd = dbg.cmd_factory.make_error_message(
                self.sequence, f"Error evaluating console expression {exc}"
            )
            dbg.writer.add_command(cmd)
        finally:
...
class InternalLoadFullValue(InternalThreadCommand):
    ...
    def do_it(self, dbg):
        ...
        try:
            ...
            sys.stderr.write('%s\n' % (exc,))
            cmd = dbg.cmd_factory.make_error_message(
                self.sequence, f"Error evaluating variable {exc} "
            )
            dbg.writer.add_command(cmd)
