...
class _TaskletInfo:
    ...
    def update_name(self):
        if tasklet := self.tasklet_weakref():
            if tasklet.blocked:
                state = 'blocked'
            elif tasklet.paused:
                state = 'paused'
            ...
            try:
                ...
                name = 'MainTasklet' if tasklet.is_main else f'Tasklet-{self._tasklet_id}'
            ...
            if thread_id != -1:
                thread_name = next(
                    (
                        f"of {thread.name}"
                        if thread.name
                        else f"of Thread-{thread.name or str(thread_id)}"
                        for thread in threading.enumerate()
                        if thread.ident == thread_id
                    ),
                    f"of Thread-{str(thread_id)}",
                )
                thread = None
            else:
            ...
            state = 'dead'
            name = f'Tasklet-{self._tasklet_id}'
            thread_name = ""
            tid = '-'
        self.tasklet_name = f'{state} {name} {thread_name} ({tid})'
    ...
    if not hasattr(stackless.tasklet, "trace_function"):
        ...
        def update_name(self):
            if tasklet := self.tasklet_weakref():
                ...
                thread_name = ""
                tid = '-'
            self.tasklet_name = f'{name} {thread_name} ({tid})'
        ...
        try:
            for tasklet_ref, tasklet_info in list(_weak_tasklet_registered_to_info.items()):  # Make sure it's a copy!
                ...
                if tasklet is None or not tasklet.alive:
                    ...
                    elif tasklet_info.frame_id is not None:
        ...
        try:
            ...
            if next:
                ...
                if (
                    ...
                    and next.frame
                    and hasattr(next.frame, 'f_trace')
                ):
