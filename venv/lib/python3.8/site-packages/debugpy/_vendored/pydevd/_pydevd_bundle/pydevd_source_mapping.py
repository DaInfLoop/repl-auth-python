...
class SourceMappingEntry(object):
    ...
    def __str__(self):
        return f"SourceMappingEntry({', '.join('%s=%r' % (attr, getattr(self, attr)) for attr in self.__slots__)})"
...
class SourceMapping(object):
    ...
    def set_source_mapping(self, absolute_filename, mapping):
        ...
        for map_entry in mapping:
            ...
            if existing_source_filename and existing_source_filename != absolute_filename:
                return f'Cannot apply mapping from {absolute_filename} to {map_entry.runtime_source} (it conflicts with mapping: {existing_source_filename} to {map_entry.runtime_source})'
    ...
    def map_to_client(self, runtime_source_filename, lineno):
        ...
        try:
            ...
            for _, mapping in list(self._mappings_to_server.items()):
                for map_entry in mapping:
                    if (
                        map_entry.runtime_source == runtime_source_filename
                        and map_entry.contains_runtime_line(lineno)
                    ):
    ...
    def map_to_server(self, absolute_filename, lineno):
        ...
        changed = False
        if mappings := self._mappings_to_server.get(absolute_normalized_filename):
            i = bisect.bisect(KeyifyList(mappings, lambda entry:entry.line), lineno)
            if i >= len(mappings):
                i -= 1

            entry = mappings[i] if i == 0 else mappings[i - 1]
