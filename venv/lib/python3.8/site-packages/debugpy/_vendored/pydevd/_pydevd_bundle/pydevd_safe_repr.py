...
class SafeRepr(object):
        ...
        if level >= len(self.maxcollection):
            ...
            for item in obj:
                ...
                if (
                    ...
                    and count == self.maxcollection[level] - 1
                ):
                    yield ','
        yield suffix
    ...
    def _repr_str(self, obj, level):
        try:
            ...
            part2 = repr(part2)
            yield part1[:part1.rindex("'")]
            yield '...'
            yield part2[part2.index("'") + 1:]
        except:
    ...
    def _bytes_as_unicode_if_possible(self, obj_repr):
        ...
        try_encodings = []
        if encoding := self.sys_stdout_encoding or getattr(
            sys.stdout, 'encoding', ''
        ):
            try_encodings.append(encoding.lower())

        if (
            preferred_encoding := self.locale_preferred_encoding
