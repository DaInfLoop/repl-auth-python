...
def get_text_list_for_frame(frame):
    ...
    try:
        while curFrame:
            ...
            cmdTextList.extend(
                (
                    f'<frame id="{myId}" name="{pydevd_xml.make_valid_xml_value(myName)}" ',
                    'file="%s" line="%s">'
...
def send_concurrency_message(event_class, time, name, thread_id, type, event, file, line, frame, lock_id=0, parent=None):
    ...
    cmdTextList = [
        '<xml>',
        f'<{event_class}',
        f' time="{pydevd_xml.make_valid_xml_value(str(time))}"',
    ]

    cmdTextList.append(f' name="{pydevd_xml.make_valid_xml_value(name)}"')
    cmdTextList.append(
        f' thread_id="{pydevd_xml.make_valid_xml_value(thread_id)}"'
    )
    cmdTextList.append(f' type="{pydevd_xml.make_valid_xml_value(type)}"')
    if type == "lock":
        cmdTextList.append(
            f' lock_id="{pydevd_xml.make_valid_xml_value(str(lock_id))}"'
        )
    if parent is not None:
        cmdTextList.append(f' parent="{pydevd_xml.make_valid_xml_value(parent)}"')
    cmdTextList.append(f' event="{pydevd_xml.make_valid_xml_value(event)}"')
    cmdTextList.append(f' file="{pydevd_xml.make_valid_xml_value(file)}"')
    cmdTextList.append(f' line="{pydevd_xml.make_valid_xml_value(str(line))}"')
    cmdTextList.append(f'></{event_class}>')
    ...
    def log_event(self, frame):
        ...
        if hasattr(frame, "f_back") and frame.f_back is not None:
            ...
            if hasattr(back, "f_back") and back.f_back is not None:
                back = back.f_back
                if "self" in back.f_locals and isinstance(
                    back.f_locals["self"], threading.Thread
                ):
                    write_log = True
        try:
            if write_log:
                ...
                if (
                    method_name == "pydev_after_run_call"
                    ...
                    if "self" in back.f_locals and isinstance(
                        ...
                        send_massage = not hasattr(my_self_obj, "_pydev_join_called")
                        if send_massage:
                ...
                if self_obj.__class__ == ObjectWrapper:
                    ...
                    if method_name == "__init__":
                        send_concurrency_message(
                            ...
                            back.f_lineno,
                            back,
                            lock_id=id(frame.f_locals["self"]),
                        )
...
class NameManager:
    ...
    def get(self, id):
        if id not in self.tasks:
            self.last += 1
            self.tasks[id] = f"{self.prefix}-{self.last}"
        ...
        if "self" in frame.f_locals:
            ...
            if isinstance(self_obj, asyncio.Task):
                ...
                if method_name == "set_result":
                    task_id = id(self_obj)
                    task_name = self.task_mgr.get(task_id)
            ...
            method_name = frame.f_code.co_name
            if isinstance(self_obj, asyncio.Lock) and method_name in (
                "acquire",
                "release",
            ):
                ...
                if method_name == "acquire":
                    if not self_obj._waiters and not self_obj.locked():
                        send_concurrency_message(
                            ...
                            task_name,
                            "lock",
                            f"{method_name}_begin",
                            frame.f_code.co_filename,
                            frame.f_lineno,
                            frame,
                            lock_id=id(self_obj),
                        )
                    method_name += "_begin" if self_obj.locked() else "_end"
            ...
            if isinstance(self_obj, asyncio.Queue) and method_name in (
                "put",
                "get",
                "_put",
                "_get",
