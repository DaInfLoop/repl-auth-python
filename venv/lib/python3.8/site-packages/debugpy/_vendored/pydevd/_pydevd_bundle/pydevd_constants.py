...
CYTHON_SUPPORTED = python_implementation == 'CPython'
#=======================================================================================================================
# Python 3?
#=======================================================================================================================
...
def is_true_in_env(env_key):
    if isinstance(env_key, tuple):
        return any(is_true_in_env(v) for v in env_key)
    else:
...
def as_float_in_env(env_key, default):
    ...
    try:
        ...
        raise RuntimeError(
            f'Error: expected the env variable: {env_key} to be set to a float value. Found: {value}'
        )
...
def as_int_in_env(env_key, default):
    ...
    try:
        ...
        raise RuntimeError(
            f'Error: expected the env variable: {env_key} to be set to a int value. Found: {value}'
        )
...
if USE_CYTHON_FLAG is not None:
    ...
    if USE_CYTHON_FLAG not in ENV_TRUE_LOWER_VALUES and USE_CYTHON_FLAG not in ENV_FALSE_LOWER_VALUES:
        raise RuntimeError(
            f'Unexpected value for PYDEVD_USE_CYTHON: {USE_CYTHON_FLAG} (enable with one of: {ENV_TRUE_LOWER_VALUES}, disable with one of: {ENV_FALSE_LOWER_VALUES})'
        )

elif not CYTHON_SUPPORTED:
    USE_CYTHON_FLAG = 'no'
...
if IS_JYTHON:
    ...
    class ForkSafeLock(object):
        ...
        def _init(self):
            self._lock = threading.RLock() if self._rlock else thread.allocate_lock()
...
def iter_chars(b):
    ...
    if isinstance(b, bytes):
        ...
        return iter(struct.unpack(f'{len(b)}c', b))
    return iter(b)
...
def set_protocol(protocol):
    ...
    ), f'Protocol ({protocol}) should be one of: {expected}'
