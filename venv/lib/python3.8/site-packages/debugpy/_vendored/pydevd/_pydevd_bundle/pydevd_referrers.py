...
def print_var_node(xml_node, stream):
    ...
    if found_as:
        stream.write(f', Found as: {unquote_plus(found_as)}')
    stream.write('\n')
    ...
    try:
        try:
            if searched_obj is None:
                ret = [
                    ...
                    pydevd_xml.var_to_xml(
                        ...
                        additional_in_xml=f' id="{id(searched_obj)}"',
                    ),
                    '</for>\n',
                    '</xml>',
                ]
            ...
            ret = [
                ...
                *(
                    pydevd_xml.var_to_xml(
                        searched_obj, f'Referrers of obj with id="{obj_id}"'
                    ),
                    '</for>\n',
                ),
            ]
            ...
            for r in referrers:
                ...
                if r_type == frame_type:
                    ...
                    if dct := getattr(r, '__dict__', None):
                        # Try to check if it's a value in the dict (and under which key it was found)
                ...
                if found_as:
                    ...
                    found_as = f' found_as="{pydevd_xml.make_valid_xml_value(found_as)}"'

                ret.append(
                    pydevd_xml.var_to_xml(
                        r,
                        representation,
                        additional_in_xml=f' id="{r_id}"{found_as}',
                    )
                )
        finally:
            if DEBUG:
