...
def _convert_rules_to_exclude_filters(rules, on_error):
    ...
    if not isinstance(rules, list):
        on_error(f'Invalid "rules" (expected list of dicts). Found: {rules}')
        ...
        for rule in rules:
            ...
            if include is None:
                on_error(f'Invalid "rule" (expected dict with "include"). Found: {rule}')
                continue
            ...
            if path is None and module is None:
                on_error(
                    f'Invalid "rule" (expected dict with "path" or "module"). Found: {rule}'
                )
                continue
            ...
            if path is not None:
                ...
                if '*' not in glob_pattern and '?' not in glob_pattern:
                    if os.glob_pattern.isdir(glob_pattern):
...
class PyDevJsonCommandProcessor(object):
    ...
    def on_pydevdauthorize_request(self, py_db, request):
        ...
        if client_access_token := py_db.authentication.client_access_token:
    ...
    def on_completions_request(self, py_db, request):
        ...
        line = -1 if arguments.line is None else arguments.line - 1
    ...
    def _set_debug_options(self, py_db, args, start_reason):
        ...
        if isinstance(variable_presentation, dict):
            ...
            def get_variable_presentation(setting, default):
                ...
                if value not in ('group', 'inline', 'hide'):
                    pydev_log.info(
                        f'The value set for "{setting}" ({value}) in the variablePresentation is not valid. Valid values are: "group", "inline", "hide"'
                    )
                    value = default
        ...
        if not isinstance(auto_reload, dict):
            pydev_log.info(f'Expected autoReload to be a dict. Received: {auto_reload}')
            auto_reload = {}
        ...
        if target_id is not None:
            ...
            if variant is not None:
                ...
                variables_response = pydevd_base_schema.build_response(
                    ...
                    kwargs={
                        'success': False,
                        'message': f'Unable to find step into target {target_id}. Available targets: {target_id_to_smart_step_into_variant}',
                    },
                )
                return NetCommand(CMD_RETURN, 0, variables_response, is_json=True)
    ...
    def on_setbreakpoints_request(self, py_db, request):
        ...
        for source_breakpoint in arguments.breakpoints:
            ...
            if log_message := source_breakpoint.logMessage:
                is_logpoint = True
    ...
    def on_setexceptionbreakpoints_request(self, py_db, request):
        ...
        if exception_options := arguments.exceptionOptions:
            break_raised = False
                ...
                if len(exception_paths) == 0:
                    ...
                    if 'Python Exceptions' in next(path_iterator)['names']:
                        for path in path_iterator:
                            exception_names.extend(iter(path['names']))
    ...
    def on_source_request(self, py_db, request):
        ...
        if content is None:
            if source_reference == 0:
                ...
                message = f'Unable to retrieve source for {server_filename}'
            else:
                message = 'Invalid sourceReference %d' % (source_reference,)
            response_args |= {'success': False, 'message': message}
    ...
    def on_setpydevdsourcemap_request(self, py_db, request):
        ...
        if error_msg := self.api.set_source_mapping(py_db, path, new_mappings):
            response = pydevd_base_schema.build_response(
                request,
                kwargs={
                    'body': {},
