...
def is_unhandled_exception(container_obj, py_db, frame, last_raise_line, raise_lines):
    ...
    return (
        any(
            try_except_info.is_line_in_except_block(frame.f_lineno)
            and (
                frame.f_lineno == try_except_info.except_line
                or frame.f_lineno in try_except_info.raise_lines_in_except
            )
...
# ENDIF
    ...
    # ENDIF
        if event == 'exception':
            ...
            if should_stop and self._handle_exception(
                frame, event, arg, EXCEPTION_TYPE_HANDLED
            ):
            ...
            if trace is not None and hasattr(trace, 'tb_next'):
                ...
                try:
                    if main_debugger.plugin is not None:
                        if result := main_debugger.plugin.exception_break(
                ...
                if not should_stop:
                    ...
                    if exception == SystemExit and main_debugger.ignore_system_exit_code(value):
                    ...
                    elif not ignore_exception_trace(trace):
                        ...
                        for exc_break, is_user_uncaught in check_excs:
                            ...
                            if main_debugger.exclude_exception_by_filter(exc_break, trace):
                                pydev_log.debug(
                                    f"Ignore exception {exception} in library {frame.f_code.co_filename} -- ({frame.f_code.co_name})"
                                )
                                should_stop = False
                                ...
                                if not main_debugger.apply_files_filter(frame, frame.f_code.co_filename, True) \
                                    ...
                                    if exc_info:
                                        ...
                                        exc_info = arg, frame.f_lineno, {frame.f_lineno}
    ...
    # ENDIF
        ...
        try:
            ...
            if trace_obj.tb_next is not None or trace_obj.tb_frame is not frame:
            ...
            if main_debugger.ignore_exceptions_thrown_in_lines_with_ignore_exception:
                for check_trace_obj in (initial_trace_obj, trace_obj):
                    ...
                    if from_user_input := main_debugger.filename_to_lines_where_exceptions_are_ignored.get(
                    ...
                    if exc_lineno not in merged:  # Note: check on merged but update lines_ignored.
                        ...
                        if IGNORE_EXCEPTION_TAG.match(line) is None:
    ...
    # ENDIF
        ...
        try:
            ...
            return f"{cls_name}.{func_name}" if cls_name is not None else func_name
        except:
            pydev_log.exception()
            return func_name
