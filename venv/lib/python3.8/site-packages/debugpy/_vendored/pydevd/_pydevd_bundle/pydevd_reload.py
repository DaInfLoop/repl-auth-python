...
class Reload:
    ...
    def _update(self, namespace, name, oldobj, newobj, is_class_namespace=False):
        ...
        try:
            ...
            classtype = (types.ClassType, type) if hasattr(types, 'ClassType') else type
            ...
            notify_error(
                f'Exception found when updating {name}. Proceeding for other items.'
            )
            pydev_log.exception()
    ...
    def _update_class(self, oldclass, newclass):
        ...
        if str(old_bases) != str(new_bases):
            notify_error(
                f'Changing the hierarchy of a class is not supported. {oldclass} may be inconsistent.'
            )
