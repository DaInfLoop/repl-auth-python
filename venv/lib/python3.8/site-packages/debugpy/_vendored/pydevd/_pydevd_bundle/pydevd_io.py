...
class IORedirector:
    ...
    def isatty(self):
        return next(
            (r.isatty() for r in self._redirect_to if hasattr(r, 'isatty')), False
        )
...
def start_redirect(keep_original_redirection=False, std='stdout', redirect_to=None):
    ...
    with _RedirectionsHolder._lock:
        ...
        config_stds = ['stdout', 'stderr'] if std == 'both' else [std]
        for std in config_stds:
            original = getattr(sys, std)
            stack = getattr(_RedirectionsHolder, f'_stack_{std}')

            if keep_original_redirection:
                wrap_buffer = bool(hasattr(redirect_to, 'buffer'))
...
def redirect_stream_to_pydb_io_messages(std):
    ...
    '''
    with _RedirectionsHolder._lock:
        redirect_to_name = f'_pydevd_{std}_redirect_'
