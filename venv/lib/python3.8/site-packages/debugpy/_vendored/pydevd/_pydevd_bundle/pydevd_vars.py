...
def getVariable(dbg, thread_id, frame_id, scope, attrs):
    ...
    attrList = attrs.split('\t') if attrs is not None else []
...
def resolve_var_object(var, attrs):
    ...
    """
    attr_list = attrs.split('\t') if attrs is not None else []
    for k in attr_list:
            ...
            if '.__' in expression:
                ...
                for entry in split[1:]:
                    if entry.startswith('__') and not hasattr(curr, entry):
                        entry = f'_{curr.__class__.__name__}{entry}'
...
def change_attr_expression(frame, attr, expression, dbg, value=SENTINEL_VALUE):
    ...
    try:
        ...
        if dbg.plugin and value is SENTINEL_VALUE:
            if result := dbg.plugin.change_variable(frame, attr, expression):
                return result
        ...
        if attr[:7] == "Globals":
            ...
            if (
                '.' not in attr
                and pydevd_save_locals.is_save_locals_available()
            ):
                if value is SENTINEL_VALUE:
                    value = eval(expression, frame.f_globals, frame.f_locals)
            ...
            if value is SENTINEL_VALUE:
                value = eval(expression, frame.f_globals, frame.f_locals)
            result = value
            Exec(f'{attr}={expression}', frame.f_globals, frame.f_locals)
            return result
...
def table_like_struct_to_xml(array, name, roffset, coffset, rows, cols, format):
    ...
    if type_name == 'ndarray':
        ...
        raise VariableError(f"Do not know how to convert type {type_name} to table")

    return f"<xml>{xml}</xml>"
...
def array_to_meta_xml(array, name, format):
    ...
    reslice = ""
    if l > 2:
        raise Exception(f"{slice} has more than 2 dimensions.")
    elif l == 1:
    ...
    bounds = (array.min(), array.max()) if type in "biufc" else (0, 0)
...
def dataframe_to_xml(df, name, roffset, coffset, rows, cols, format):
    ...
    if (num_rows, num_cols) != df.shape:
        ...
        slice = f'.iloc[0:{num_rows}, 0:{num_cols}]'
    else:
        slice = ''
    slice = name + slice
    ...
    get_label = (
        lambda label: '/'.join(map(str, label))
