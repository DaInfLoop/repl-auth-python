...
class _PyDevCommandProcessor(object):
    ...
    def process_net_command(self, py_db, cmd_id, seq, text):
        ...
        if on_command is None:
            ...
            cmd = py_db.cmd_factory.make_error_message(
                seq, f"unexpected command {str(cmd_id)}"
            )
            py_db.writer.add_command(cmd)
            return

        lock = NULL if method_name == 'cmd_thread_dump_to_stderr' else py_db._main_lock
        with lock:
            try:
    ...
    def cmd_version(self, py_db, cmd_id, seq, text):
        ...
        # provide it).
        ide_os = 'WINDOWS' if IS_WINDOWS else 'UNIX'
    ...
    def cmd_smart_step_into(self, py_db, cmd_id, seq, text):
        ...
        if not line_or_bytecode_offset.startswith('offset='):
    ...
    def cmd_change_variable(self, py_db, cmd_id, seq, text):
        ...
        attr = attr_and_value[:tab_index].replace('\t', '.')
    ...
    def cmd_set_py_exception(self, py_db, cmd_id, seq, text):
        ...
        try:
            ...
            py_db.break_on_user_uncaught_exceptions = {}
            if len(splitted) >= 5:
                break_on_uncaught = splitted[0] == 'true'
                break_on_caught = splitted[1] == 'true'
                py_db.skip_on_exceptions_thrown_in_same_context = splitted[2] == 'true'
                py_db.ignore_exceptions_thrown_in_lines_with_ignore_exception = (
                    splitted[3] == 'true'
                )
                ignore_libraries = splitted[4] == 'true'
                for exception_type in splitted[5:]:
                    if exception_type := exception_type.strip():
                        py_db.add_break_on_exception(
                            exception_type,
                            condition=None,
                            expression=None,
    ...
    def cmd_set_property_trace(self, py_db, cmd_id, seq, text):
        ...
        if len(splitted) >= 3:
            ...
            py_db.disable_property_getter_trace = splitted[1] == 'true'
                # Enable/Disable tracing of the property setter
            py_db.disable_property_setter_trace = splitted[2] == 'true'
                # Enable/Disable tracing of the property deleter
            py_db.disable_property_deleter_trace = splitted[3] == 'true'
    ...
    def cmd_evaluate_console_expression(self, py_db, cmd_id, seq, text):
        ...
        if text != "":
            ...
            if console_command == 'EVALUATE':
                ...
                raise ValueError(f'Unrecognized command: {console_command}')
