...
# ENDIF
    ...
    def trace_unhandled_exceptions(self, frame, event, arg):
        ...
        if event == 'exception' and arg is not None:
            py_db, t, additional_info = self._args[:3]
    ...
    def trace_dispatch_and_unhandled_exceptions(self, frame, event, arg):
        ...
        if event == 'exception':
            ...
            try:
                py_db, t, additional_info = self._args[:3]
                if (
                    not additional_info.suspended_at_unhandled
                    and is_unhandled_exception(
                        self,
                        py_db,
                        frame,
                        self._last_raise_line,
                        self._raise_lines,
                    )
                ):
        ...
        try:
            ...
            if frame_cache_key in cache_skips:
                ...
                if cache_skips.get(frame_cache_key) == 1:
                    ...
                    if back_frame is None or pydev_step_cmd not in (
                        CMD_STEP_INTO,
            ...
            if py_db.is_files_filter_enabled and py_db.apply_files_filter(
                frame, abs_path_canonical_path_and_base[0], False
            ):
                cache_skips[frame_cache_key] = 1
                ...
                if py_db.apply_files_filter(back_frame, back_frame.f_code.co_filename, False):
                    cache_skips[back_frame.f_code] = 1
