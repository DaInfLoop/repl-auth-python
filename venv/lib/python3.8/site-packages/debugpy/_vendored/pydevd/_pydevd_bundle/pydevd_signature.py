...
class Signature(object):
    ...
    def add_arg(self, name, type):
        ...
        self.args_str.append(f"{name}:{type}")
    ...
    def __str__(self):
        return f'{self.file} {self.name}({", ".join(self.args_str)})'
...
def get_type_of_value(value, ignore_module_name=('__main__', '__builtin__', 'builtins'), recursive=False):
    ...
    if hasattr(tp, '__module__') and tp.__module__ and tp.__module__ not in ignore_module_name:
        class_name = f"{tp.__module__}.{class_name}"
...
class CallSignatureCache(object):
    ...
    def is_in_cache(self, signature):
        filename, name, args_type = get_signature_info(signature)
        return args_type in self.cache.get(filename, {}).get(name, {})


def create_signature_message(signature):
    cmdTextList = [
        "<xml>",
        f'<call_signature file="{pydevd_xml.make_valid_xml_value(signature.file)}" name="{pydevd_xml.make_valid_xml_value(signature.name)}">',
    ]

    cmdTextList.extend(
        f'<arg name="{pydevd_xml.make_valid_xml_value(arg[0])}" type="{pydevd_xml.make_valid_xml_value(arg[1])}"></arg>'
        for arg in signature.args
    )
    if signature.return_type is not None:
        cmdTextList.append(
            f'<return type="{pydevd_xml.make_valid_xml_value(signature.return_type)}"></return>'
        )
    ...
    if dbg.signature_factory and dbg.in_project_scope(frame):
        ...
        if signature is not None:
            if dbg.signature_factory.cache is not None:
                if dbg.signature_factory.cache.is_in_cache(signature):
