        ...
        for provider in self._str_providers:
            if provider.can_provide(type_object, type_name):
                ...
                try:
                    ...
                    pydev_log.exception(
                        f"Error when getting str with custom provider: {provider}."
                    )
...
def return_values_from_dict_to_xml(return_dict):
    res = [
        var_to_xml(val, name, additional_in_xml=' isRetVal="True"')
...
def get_variable_details(val, evaluate_full_value=True, to_string=None):
    ...
    v = val.result if is_exception_on_eval else val
        ...
        try:
            ...
            if str_from_provider is not None:
                ...
                if v.__class__ == frame_type:
                    ...
                    if len(v) > 300:
                        value = f'{str(v.__class__)}: <Too big to print. Len: {len(v)}>'
                    else:
                        value = f'{str(v.__class__)}: {v}'
                else:
                    try:
            ...
            try:
                ...
                value = f'Unable to get repr for {v.__class__}'
...
def var_to_xml(val, name, trim_if_too_big=True, additional_in_xml='', evaluate_full_value=True):
    ...
    xml = f'<var name="{make_valid_xml_value(name)}" type="{make_valid_xml_value(type_name)}" '

    if type_qualifier:
        xml_qualifier = f'qualifier="{make_valid_xml_value(type_qualifier)}"'
    else:
        xml_qualifier = ''
    ...
    if value:
        ...
        if len(value) > MAXIMUM_VARIABLE_REPRESENTATION_SIZE and trim_if_too_big:
            value = value[:MAXIMUM_VARIABLE_REPRESENTATION_SIZE]
            value += '...'

        xml_value = f""" value="{make_valid_xml_value(quote(value, '/>_= '))}\""""
    else:
        xml_value = ''
    ...
    if is_exception_on_eval:
        ...
        xml_container = ' isContainer="True"' if resolver is not None else ''
    if scope:
