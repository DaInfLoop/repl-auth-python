...
def _convert_to_str_and_clear_empty(roots):
    new_roots = []
    for root in roots:
        assert isinstance(root, str), f'{root} not str (found: {type(root)})'
        if root:
...
def glob_matches_path(path, pattern, sep=os.sep, altsep=os.altsep):
    ...
    if drive and len(pattern) > 1 and pattern[1] == ':':
...
class FilesFiltering(object):
    ...
    def _get_default_library_roots(cls):
        ...
        try:
            ...
            roots.extend(
                ...
                & set(sysconfig.get_path_names())
            )
        roots.extend(
            (os.path.dirname(os.__file__), os.path.dirname(threading.__file__))
        )
        if IS_PYPY:
        ...
        roots.extend(
            ...
            and os.path.basename(path) in ('site-packages', 'pip-global')
        )
    ...
    def exclude_by_filter(self, absolute_filename, module_name):
        ...
        for exclude_filter in self._exclude_filters:  # : :type exclude_filter: ExcludeFilter
            ...
            elif exclude_filter.name == module_name or module_name.startswith(
                f'{exclude_filter.name}.'
            ):
                return exclude_filter.exclude
        return None
    ...
    def set_exclude_filters(self, exclude_filters):
        ...
        '''
        self._exclude_filters = exclude_filters
        self.require_module = any(
            not exclude_filter.is_path for exclude_filter in exclude_filters
        )
