...
__version_info_str__ = [str(v) for v in __version_info__]
...
class AbstractSingleNotificationBehavior(object):
    ...
    def _notify_after_timeout(self, global_suspend_time):
        with self._lock:
            if (
                self._suspended_thread_ids
                and global_suspend_time > self._last_suspend_notification_time
            ):
...
class PyDB(object):
    ...
    def handle_breakpoint_condition(self, info, pybreakpoint, new_frame):
        ...
        try:
            ...
            return (
                eval(condition, new_frame.f_globals, new_frame.f_locals)
                if condition
                else False
            )
    ...
    def get_file_type(self, frame, abs_real_path_and_basename=None, _cache_file_type=_CACHE_FILE_TYPE):
        ...
        try:
            ...
            if file_type is None and self.dont_trace_external_files(
                abs_real_path_and_basename[0]
            ):
    ...
    def set_tracing_for_untraced_contexts(self):
        ...
        if IS_CPYTHON:
            ...
            ignore_thread_ids = {
                ...
                or getattr(t, 'pydev_do_not_trace', False)
            }
    ...
    def in_project_scope(self, frame, absolute_filename=None):
        ...
        try:
            ...
            if file_type == self.PYDEV_FILE:
                ...
                cache[cache_key] = file_type != self.LIB_FILE
    ...
    def apply_files_filter(self, frame, original_filename, force_check_project_scope):
        ...
        try:
            ...
            if (
                ...
                and (
                    self.has_plugin_line_breaks or self.has_plugin_exception_breaks
                )
                and not self.plugin.can_skip(self, frame)
            ):
    ...
    def exclude_exception_by_filter(self, exception_breakpoint, trace):
        ...
        return bool(
            (ignore_libraries and not self.in_project_scope(trace.tb_frame))
            or (
                exclude_filters_enabled
                and self._exclude_by_filter(
                    trace.tb_frame,
                    pydevd_file_utils.absolute_path(
    ...
    def has_user_threads_alive(self):
        for t in pydevd_utils.get_non_pydevd_threads():
            ...
            if is_thread_alive(t) and (
                not t.daemon or hasattr(t, "__pydevd_main_thread")
            ):
                return True
    ...
    def connect(self, host, port):
        s = start_client(host, port) if host else start_server(port)
    ...
    def _activate_gui_if_needed(self):
        ...
        if (
            len(self.mpl_modules_for_patching) > 0
            and is_current_thread_main_thread()
        ):
            for module in list(self.mpl_modules_for_patching):
        ...
        if self.activate_gui_function and is_current_thread_main_thread():
            try:
                ...
                try:
                    ...
                    pydev_log.debug(
                        f"Cannot activate custom GUI event loop {self._gui_event_loop}: {e}"
                    )
                finally:
                    self.activate_gui_function = None
    ...
    def process_internal_commands(self):
        ...
        with self._main_lock:
            ...
            if not program_threads_alive and ready_to_run:
                dispose = True
            else:
                ...
                process_thread_ids = (curr_thread_id, '*') if ready_to_run else ('*', )
    ...
    def consolidate_breakpoints(self, canonical_normalized_filename, id_to_breakpoint, file_to_line_to_breakpoints):
        break_dict = {
            pybreakpoint.line: pybreakpoint
    ...
    def _do_wait_suspend(self, thread, frame, event, arg, suspend_type, from_this_thread, frames_tracker):
        ...
        self.cancel_async_evaluation(get_current_thread_id(thread), id(frame))
        ...
        if info.pydev_step_cmd in (CMD_STEP_INTO, CMD_STEP_INTO_MY_CODE):
            ...
            if (
                force_check_project_scope := info.pydev_step_cmd
                == CMD_STEP_RETURN_MY_CODE
            ):
                while back_frame is not None and self.apply_files_filter(
                    back_frame,
            ...
            elif self.is_files_filter_enabled:
                while back_frame is not None and self.apply_files_filter(
                    back_frame,
                    back_frame.f_code.co_filename,
                    force_check_project_scope,
        ...
        if is_module:
            ...
            if filename is None:
                ...
                if mod_dir is None:
                    ...
                    filename = get_fullname(f"{module_name}.__main__")
                    if filename is None:
                        sys.stderr.write("No module named %s\n" % file)
                        return
                    else:
...
def _enable_attach(
    ...
    if SetupHolder.setup is not None and port != SetupHolder.setup['port']:
        raise AssertionError(
            f"Unable to listen in port: {port} (already listening in port: {SetupHolder.setup['port']})"
        )
    settrace(
        host=host,
        port=port,
...
def settrace_forked(setup_tracing=True):
    ...
    if setup_tracing and port is not None:
