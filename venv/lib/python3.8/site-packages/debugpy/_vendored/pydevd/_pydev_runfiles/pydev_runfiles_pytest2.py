...
def _load_filters():
    ...
    if py_test_accept_filter is None:
        if py_test_accept_filter := os.environ.get('PYDEV_PYTEST_SKIP'):
            ...
            new_dct = {
                canonical_normalized_path(str(Path(filename).resolve())): value
...
def connect_to_server_for_communication_to_xml_rpc_on_xdist():
    ...
    connected = True
    if is_in_xdist_node():
        if port := os.environ.get('PYDEV_PYTEST_SERVER'):
    ...
    for item in items:
        ...
        for test in accept_tests:
            ...
            if class_name is not None:
                if test == f'{class_name}.{name}':
                    new_items.append(item)
                    break

                if (
                    name_without_parametrize is not None
                    and test == f'{class_name}.{name_without_parametrize}'
                ):
                    new_items.append(item)
                    break
...
def pytest_collectreport(report):
    if error_contents := _get_error_contents_from_report(report):
...
def pytest_runtest_logreport(report):
    ...
    if hasattr(report, 'wasxfail') and report_outcome != 'skipped':
    ...
    if report_outcome == 'passed':
        ...
        status = 'error' if report_when in ('setup', 'teardown') else 'fail'
    ...
    if report_outcome != 'skipped':
        if exc := _get_error_contents_from_report(report):
            if error_contents:
...
if not hasattr(pytest, 'hookimpl'):
    raise AssertionError(
        f'Please upgrade pytest (the current version of pytest: {pytest.__version__} is unsupported)'
    )
