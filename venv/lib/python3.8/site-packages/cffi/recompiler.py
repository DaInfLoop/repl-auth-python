...
class FieldExpr:
    ...
    def as_field_python_expr(self):
        if self.field_type_op.op == OP_NOOP:
            ...
            raise NotImplementedError
        return f"b'{self.field_type_op.as_python_bytes()}{size_expr}{self.name}'"
...
class StructUnionExpr:
    ...
    def as_python_expr(self):
        ...
        return f"(b'{format_four_bytes(self.type_index)}{format_four_bytes(flags)}{self.name}',{','.join(fields_expr)})"
...
class Recompiler:
    ...
    def collect_type_table(self):
        ...
        for tp in all_decls:
            method = getattr(self, f'_emit_bytecode_{tp.__class__.__name__}')
    ...
    def collect_step_tables(self):
        ...
        self._lsts = {step_name: [] for step_name in self.ALL_STEPS}
    ...
    def write_c_source_to_f(self, f, preamble):
        ...
        if self.ffi._embedding is not None:
            ...
            prnt(f'# define _CFFI_PYTHON_STARTUP_FUNC  _cffi_pypyinit_{base_module_name}')
        ...
        for i, op in enumerate(self.cffi_types):
            ...
            if i in typeindex2type:
                comment = f' // {typeindex2type[i]._get_c_name()}'
        ...
        for step_name in self.ALL_STEPS:
            if nums[step_name] > 0:
                prnt(f'  _cffi_{step_name}s,')
            else:
                prnt(f'  NULL,  /* no {step_name}s */')
        for step_name in self.ALL_STEPS:
            if step_name != "field":
    ...
    def write_py_source_to_f(self, f):
        ...
        prnt(f"    _types = {self._to_py(''.join(types_lst))},")
        typeindex2type = dict([(i, tp) for (tp, i) in self._typesdict.items()])
        #
        ...
        if num_includes > 0:
            prnt(
                f"    _includes = ({', '.join(['_ffi%d' % i for i in range(num_includes)])},),"
            )
        #
        # the footer
        prnt(')')
    ...
    def _convert_funcarg_to_c(self, tp, fromvar, tovar, errcode):
        ...
        if isinstance(tp, model.BasePrimitiveType) and not tp.is_complex_type():
            if tp.is_integer_type() and tp.name != '_Bool':
                ...
                converter = f"({tp.get_c_name('')})_cffi_to_c_double"
            else:
                cname = tp.get_c_name('')
                converter = f"({cname})_cffi_to_c_{tp.name.replace(' ', '_')}"
            ...
            converter = f"({tp.get_c_name('')})_cffi_to_c_pointer"
        ...
        self._prnt(
            f"  if ({tovar} == ({tp.get_c_name('')}){errvalue} && PyErr_Occurred())"
        )
        self._prnt(f'    {errcode};')
    ...
    def _generate_cpy_function_decl(self, tp, name):
        ...
        if not isinstance(tp.result, model.VoidType):
            ...
            result_decl = f"  {tp.result.get_c_name(' result', context)};"
            prnt(result_decl)
        ...
        if result_code:
            prnt(
                f"  pyresult = {self._convert_expr_from_c(tp.result, 'result', 'result type')};"
            )
    ...
    def _field_type(self, tp_struct, field_name, tp_field):
        if isinstance(tp_field, model.ArrayType):
            ...
            if actual_length == '...':
                ...
                actual_length = f'_cffi_array_len((({ptr_struct_name})0)->{field_name})'
            tp_item = self._field_type(tp_struct, f'{field_name}[0]', tp_field.item)
        ...
        for fname, ftype, fbitsize, fqual in self._enum_fields(tp):
            try:
                if ftype.is_integer_type() or fbitsize >= 0:
                    ...
                    if fname != '':
                        prnt(
                            f"  (void)((p->{fname}) | 0);  /* check that '{cname}.{fname}' is an integer */"
                        )
                    continue
    ...
    def _struct_ctx(self, tp, cname, approxname, named_ptr=None):
        ...
        if (tp not in self.ffi._parser._included_declarations and
            ...
            if tp.fldtypes is None:
                pass    # opaque
            elif not tp.partial and not any(tp.anonymous_struct_fields()):
        ...
        if reason_for_not_expanding is None:
            ...
            for fldname, fldtype, fbitsize, fqual in enumfields:
                ...
                if fbitsize >= 0:
                    ...
                    size = f"sizeof((({tp.get_c_name('*') if named_ptr is None else named_ptr.name})0)->{fldname})"
                if cname is None or fbitsize >= 0:
                    offset = '(size_t)-1'
                elif named_ptr is not None:
                    offset = f'((char *)&(({named_ptr.name})0)->{fldname}) - (char *)0'
                else:
                    offset = f"offsetof({tp.get_c_name('')}, {fldname})"
                c_fields.append(
    ...
    def _check_not_opaque(self, tp, location):
        ...
        if isinstance(tp, model.StructOrUnion) and tp.fldtypes is None:
            raise TypeError(f"{location} is of an opaque type (not declared in cdef())")
    ...
    def _generate_cpy_anonymous_decl(self, tp, name):
        if isinstance(tp, model.EnumType):
            ...
            self._struct_decl(tp, name, f'typedef_{name}')

    def _generate_cpy_anonymous_ctx(self, tp, name):
        if isinstance(tp, model.EnumType):
            self._enum_ctx(tp, name)
        else:
            self._struct_ctx(tp, name, f'typedef_{name}')
    ...
    def _generate_cpy_const(self, is_int, name, tp=None, category='const',
        ...
        prnt = self._prnt
        if is_int:
            prnt(f'static int {funcname}(unsigned long long *o)')
            prnt('{')
            prnt(f'  int n = ({name}) <= 0;')
            prnt(
                f'  *o = (unsigned long long)(({name}) | 0);  /* check that {name} is an integer */'
            )
            ...
            prnt(f'static void {funcname}(char *o)')
            prnt('{')
            prnt(f"  *({tp.get_c_name('*')})o = {name};")
        prnt('}')
        prnt()
    ...
    def _generate_cpy_constant_ctx(self, tp, name):
        if not self.target_is_python and tp.is_integer_type():
            ...
            const_kind = OP_DLOPEN_CONST if self.target_is_python else OP_CONSTANT
            type_index = self._typesdict[tp]
            type_op = CffiOp(const_kind, type_index)
        self._lsts["global"].append(GlobalExpr(name, f'_cffi_const_{name}', type_op))
    ...
    def _generate_cpy_macro_decl(self, tp, name):
        check_value = None if tp == '...' else tp
    ...
    def _generate_cpy_macro_ctx(self, tp, name):
        ...
        self._lsts["global"].append(
            GlobalExpr(
                name, f'_cffi_const_{name}', type_op, check_value=check_value
            )
        )
    ...
    def _global_type(self, tp, global_name):
        if isinstance(tp, model.ArrayType):
            ...
            tp_item = self._global_type(tp.item, f'{global_name}[0]')
    ...
    def _generate_cpy_variable_decl(self, tp, name):
        ...
        prnt(f'static {tp.get_c_name(decl, quals=self._current_quals)}')
        prnt('{')
    ...
    def _generate_cpy_variable_ctx(self, tp, name):
        tp = self._global_type(tp, name)
        type_index = self._typesdict[tp]
        op = OP_GLOBAL_VAR if self.target_is_python else OP_GLOBAL_VAR_F
        self._lsts["global"].append(
            GlobalExpr(name, f'_cffi_var_{name}', CffiOp(op, type_index))
        )
    ...
    def _extern_python_decl(self, tp, name, tag_and_space):
        ...
        if isinstance(tp.result, model.VoidType):
            ...
            context = f'result of {name}'
            size_of_result = f"(int)sizeof({tp.result.get_c_name('', context)})"
        prnt(f'static struct _cffi_externpy_s _cffi_externpy__{name} =')
        prnt('  { "%s.%s", %s, 0, 0 };' % (
        ...
        if tp.abi == "__stdcall":
            name_and_arguments = f'_cffi_stdcall {name_and_arguments}'
        #
        ...
        prnt(f'  _cffi_call_python(&_cffi_externpy__{name}, p);')
        if not isinstance(tp.result, model.VoidType):
            prnt(f"  return *({tp.result.get_c_name('*')})p;")
        prnt('}')
        prnt()
    ...
    def _generate_cpy_extern_python_ctx(self, tp, name):
        ...
        self._lsts["global"].append(
            GlobalExpr(name, f'&_cffi_externpy__{name}', type_op, name)
        )
